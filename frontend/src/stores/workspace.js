...
export const useWorkspaceStore = defineStore('workspace', () => {
  ...
  const agentStatuses = computed(() => {
    ...
    const names = agentSequence.value.length
      ? agentSequence.value
      : ['supervisor', 'profile', 'retrieve', 'doc', 'mindmap', 'quiz', 'code', 'reviewer']
  ...
  function setAgentRunning(sequence = []) {
    ...
    executionLog.value = agentSequence.value.map(name => ({
      node: name,
  ...
  function markAgentFailed(message = 'request failed') {
    ...
    executionLog.value = names.map((name, index) => ({
      node: name,
      status: index === 0 ? 'error' : 'idle',
      duration_ms: 0,
      error: index === 0 ? message : ''
    }))
  }
